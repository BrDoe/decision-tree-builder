...
const DEFAULT_INPUT = `Схема принятия решений  
  ...
  ASCII
    Получите готовую ASCII-схему
      Используйте опцию code, чтобы Jira корректно отображала схему
  Диаграмма
    Просмотрите визуальную диаграмму
      Масштабируйте и перемещайте
      Экспортируйте диаграмму в PNG`;
  ...
  nodes.forEach((root, idx) => {
    ...
    if (root.children.length) {
      ...
      root.children.forEach((child, cidx) => {
        const childIsLast = cidx === root.children.length - 1;
        render(child, "", childIsLast);
        // Add a spacer line between top-level branches to visually separate them.
        if (!childIsLast) out.push("│");
      });
    }
